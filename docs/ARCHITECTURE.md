...
Toda dependência externa de rede (email transacional, OAuth de terceiros) é um
gateway: uma interface + uma implementação real, registradas no plugin
`src/http/plugins/gateways-plugin.ts` e expostas em `app.gateways`.
`buildApp({ gateways })` aceita um override completo — é assim que os testes
de integração trocam email/google por fakes, sem `vi.mock` de módulo nenhum.
`test/mocks/gateways.fake.ts` exporta `buildFakeGateways()`, que devolve um
objeto com `sentEmails` (array acumulando o que seria enviado) e
...
- **Papéis reais por loja**: `AuthRepository.userRoles()` devolve `{}` até o
  Plano 2 introduzir `user_store_roles` — hoje só a persona `customer` (e
  `platform_admin` via flag) existe de fato.
- **Paginação por cursor**: nenhuma rota de listagem existe ainda. Quando
  chegar, o padrão a adotar é cursor opaco (id + campo de ordenação
  codificados em base64url), nunca `offset/limit`, para não quebrar sob
  escrita concorrente. Documentar o schema de cursor no schema Zod da rota
  quando implementado.
